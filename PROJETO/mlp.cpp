        ...
        for(j=0;j<net_layers[i];j++){
            if(i!=(net_tot_layers-1)){ // A ultima camada não possui pesos
                ...
                for(k=0;k<net_layers[i+1];k++){
                    ...
                    //cout<<Layers[i].Neurons[j].Dendrites[k].d_weight<<" ";
                }
            }
            if(i!=0){ // A primeira camada não possui bias
    ...
    for(i=1;i<net_tot_layers;i++){
        for(j=0;j<net_layers[i];j++){
            ...
            for(k=0;k<net_layers[i-1];k++){
                ...
                //cout<<Layers[i-1].Neurons[k].Dendrites[j].d_weight<<" ";
            }
...
int MLP::Train(double inputs[],double outputs[]){ // O Algoritmo de aprendizado Backprop padrão
    int i,j,k;
...
void MLP::carregarPesos(const char *arquivo){ // Salva pesos da rede
	int i,j,k,result;
	...
	{
        printf("Problemas na ABERTURA do arquivo do treinamento\n");
		return;
	}
